...
ModelVarSet::ModelVarSet(std::string model_name_) {
	model_name = model_name_;
}

ModelVarSet::~ModelVarSet() {
	// TODO Auto-generated destructor stub
}

unsigned int ModelVarSet::getNumberOfParameters() const {
	return model_par_map.size();
}

unsigned int ModelVarSet::getNumberOfFreeParameters() const {
...
void ModelVarSet::printInfo() const {
...
const shared_ptr<ModelVar> ModelVarSet::addModelParameter(std::string name_,
...
void ModelVarSet::addModelParameter(shared_ptr<ModelVar> model_par) {
	model_par_map[std::make_pair(model_name, model_par->getName())] = model_par;
}

int ModelVarSet::setModelParameterValue(const std::string &name_,
		double value_) {
	if (modelParameterExists(name_)) {
...
int ModelVarSet::addModelParameters(ModelVarSet &daughter_model_par_set) {
...
const double& ModelVarSet::getModelParameterValue(
		const std::string &name_) const {
	return model_par_map.at(std::make_pair(model_name, name_))->getValue();
}

shared_ptr<ModelVar> ModelVarSet::getModelParameter(
...
shared_ptr<ModelVar> ModelVarSet::getModelParameter(const std::string &name_) {
	if (!modelParameterExists(name_)) {
...
int ModelVarSet::checkParameters() const {
...
bool ModelVarSet::checkSuperiorParameters() const {
	for (std::map<std::pair<std::string, std::string>, shared_ptr<ModelPar>
			, ModelStructs::stringpair_comp>::const_iterator it =
			model_par_map.begin(); it != model_par_map.end(); it++) {
...
std::map<std::pair<std::string, std::string>, shared_ptr<ModelPar>
		, ModelStructs::stringpair_comp> ModelVarSet::getFreeModelParameters() const {
	std::map<std::pair<std::string, std::string>, shared_ptr<ModelPar>
...
void ModelVarSet::freeModelParameter(const std::string &name_) {
	if (modelParameterExists(name_)) {
...
void ModelVarSet::freeModelParameter(
...
void ModelVarSet::freeAllModelParameters() {
	for (std::map<std::pair<std::string, std::string>, shared_ptr<ModelPar>
			, ModelStructs::stringpair_comp>::const_iterator it =
			model_par_map.begin(); it != model_par_map.end(); it++) {
		if (!it->second->isSuperior()) {
...
std::map<std::pair<std::string, std::string>, shared_ptr<ModelPar>
		, ModelStructs::stringpair_comp>& ModelVarSet::getModelParameterMap() {
